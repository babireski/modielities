...
Sistemas podem ser definidos tanto sobre operadores de conseguimento $C:\wp(\mathcal{L})\to\wp(\mathcal{L})$ quanto sobre relações de conseguimento $\entails\: :\wp(\mathcal{L})\times\mathcal{L}$. Ambas as definições são equivalentes, uma vez que $\Gamma\entails\alpha\Leftrightarrow \alpha\in C(\Gamma)$. Desde modo, quaisquer propriedades provadas para um dos conceitos pode ser trivialmente provada para o outro.
